...
public class TaskServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    Period difference = null, diff = null;
	    if(deadline.length() != 0) {
		    ...
		    difference = Period.between(today, end_line); //difference between the dates is calculated
	    }
	    ...
	    if(start_date.length() != 0) {
		    ...
		    diff = Period.between(today, start_line);
	    }

	    if(diff != null && (diff.getYears()>0 || diff.getDays()>0 || diff.getMonths()>0)){
	    	color = "grey";
	    	status = "Inactive";
	    }else{
	    	if(difference != null && (difference.getYears()<0 || difference.getDays()<0 || difference.getMonths()<0)) {
	            color = "red";
	            status = "Late";
	        }else{
		    	color = "black";
	    ...
	    String task_msg = userService.taskValidation(taskBean);
	    
	    if(task_msg.equals("Success")) {
	    	...
	    	response.sendRedirect("admin/add_task.jsp?msg=" + task_msg);
	    }
